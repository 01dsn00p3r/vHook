...
void drawBombTimer(C_BasePlantedC4* bomb, C_BaseEntity* local){
    ...
    draw->DrawBasicString(10, 35, Color::Magenta(), espfont, bombTime);
    ...
    for(int i = 0; i < pEntList->GetHighestEntityIndex(); i++) {
        ...
        if(DrawPlayerBox(entity, players)) {
            ...
            if(entity->IsDefusing())
                draw->drawstring(players.x + players.w / 2, players.y - 27, Color::Magenta(), espfont, "DEFUSING", true);

            /* Draw skeleton */
            if(vars.visuals.skeleton)
                DrawSkeleton(entity, Color::White());
                
            if(vars.visuals.bombtimer)
                drawBombTimer(pC4, local);
