...
void cdrawings::handleinput(int eventcode) {
	if (IsKeyPressed(eventcode, KEY_BACKSPACE))
	{
		std::size_t nLength = m_szCurrentString[activeIndex].length();
		if (nLength > 0)
		{
			m_szCurrentString[activeIndex].erase(nLength - 1);
	...
	if(IsKeyPressed(eventcode, KEY_ENTER)) {
		m_szChangedValue[activeIndex] = m_szCurrentString[activeIndex];
		return;
	}
	
	if (IsKeyPressed(eventcode, KEY_SPACE))
	{
		m_szCurrentString[activeIndex].append(" ");
		return;
	}
	
	if (eventcode >= KEY_0
		&& eventcode <= KEY_EQUAL
		&& m_szCurrentString[activeIndex].length() <= 114)
	{
		if (pInputSystem->IsButtonDown(KEY_LSHIFT) || pInputSystem->IsButtonDown(KEY_RSHIFT))
		{
			m_szCurrentString[activeIndex].append(m_KeyStroke[eventcode].m_szShiftDefinition);
			m_nCurrentPos[activeIndex]++;
		}
		else
		{
			m_szCurrentString[activeIndex].append(m_KeyStroke[eventcode].m_szDefinition);
			m_nCurrentPos[activeIndex]++;
		}
	}
}
...
void cdrawings::drawmenu() { // Draws menu
	...
	this->drawstring(rewidth + 6, y + 4, Color(160, 160, 160, 255), framefont, "Hehe xD"); // Window title
	...
	if(vars.tabIndex == tTabs::Main) {
		...
		MainTabs.push_back("Aimbot");
		MainTabs.push_back("Visuals");
		MainTabs.push_back("Chams");
		MainTabs.push_back("Misc");
		MainTabs.push_back("Colors");
		MainTabs.push_back("HvH");
		MainTabs.push_back("Removals");
		MainTabs.push_back("Playerlist");
		...
		for(int i = 0; i < MainTabs.size(); i++) { // Draws main tabs
		 ...
		 if(vars.main.tab == cMainTabs::AimbotTab) {
			this->addbutton(rewidth + 12, y + 50, "Enabled", &vars.aimbot.enabled, 0);
			this->addbutton(rewidth + 12, y + 50, "Silent Aim", &vars.aimbot.silent, 1);
			this->addbutton(rewidth + 12 + 170, y + 50, "Smoothing", &vars.aimbot.smooth, 1);
			this->addslider(rewidth + 12 + 170, y + 50, 90, "", vars.aimbot.smoothf, 2, 1.f, 0.f); // Smothing amount
			this->addbutton(rewidth + 12, y + 50, "pSilent Aim", &vars.aimbot.psilent, 2);
			this->addslider(rewidth + 12, y + 50, 90, "Field Of View", vars.aimbot.fov, 3, 180, 0); // 180 and 0... 180 Are the maximum value and 0 is the minimum.
			this->addslider(rewidth + 12, y + 50, 90, "Point Scale", vars.aimbot.pointscale, 4, 100, 0); // 180 and 0... 180 Are the maximum value and 0 is the minimum.
			this->addslider(rewidth + 12, y + 50, 90, "Hitbox", vars.aimbot.bone, 5, 20, 0); // 180 and 0... 180 Are the maximum value and 0 is the minimum.
			this->addbutton(rewidth + 12, y + 50, "Hitscan", &vars.aimbot.hitscan, 6);
			this->addbutton(rewidth + 12, y + 50, "Self Prediction", &vars.aimbot.selfpred, 7);
			this->addbutton(rewidth + 12, y + 50, "Auto Wall", &vars.aimbot.autowall, 8);
			this->addbutton(rewidth + 12, y + 50, "Auto Shoot", &vars.aimbot.autoshoot, 9);
			this->addbutton(rewidth + 12 + 170, y + 50, "Auto Pistol", &vars.aimbot.autopistol, 9);
			this->addbutton(rewidth + 12, y + 50, "Auto Stop", &vars.aimbot.autostop, 10);
			this->addbutton(rewidth + 12 + 170, y + 50, "Auto Crouch", &vars.aimbot.autocrouch, 10);
			this->addbutton(rewidth + 12, y + 50, "Trigger", &vars.aimbot.trigger, 11, IFlags::IFLAG_WARNING);
		 }
		 
		 if(vars.main.tab == cMainTabs::VisualsTab) {
			this->addbutton(rewidth + 12, y + 50, "Enabled", &vars.visuals.enabled, 0);
			this->addbutton(rewidth + 12, y + 50, "Box", &vars.visuals.box, 1);
			this->addbutton(rewidth + 12 + 170, y + 50, "Skin changer", &vars.visuals.skinc, 1);
			this->addbutton(rewidth + 12, y + 50, "Name", &vars.visuals.name, 2);
			this->addbutton(rewidth + 12, y + 50, "Health Text", &vars.visuals.healthtext, 3);
			this->addbutton(rewidth + 12, y + 50, "Health", &vars.visuals.health, 4); // Doesn't work
			this->addbutton(rewidth + 12, y + 50, "Snaplines", &vars.visuals.snapline, 5);
			this->addbutton(rewidth + 12, y + 50, "Skeleton", &vars.visuals.skeleton, 6);
			this->addbutton(rewidth + 12, y + 50, "Hitbox", &vars.visuals.headhitbox, 7);
			this->addbutton(rewidth + 12, y + 50, "Enemy Only", &vars.visuals.enemyonly, 8);
			
		 }
		 
		 if(vars.main.tab == cMainTabs::ChamsTab) {
			std::vector<std::string> chamsvector;
			std::vector<std::string> handsvector;
			...
			this->addbutton(rewidth + 12, y + 50, "Chams", &vars.visuals.chams, 0);
			this->addbutton(rewidth + 12 + 170, y + 50, "Hands", &vars.visuals.handchams, 0);
			
			this->addcombo(rewidth + 12 + 170, y + 50, 90, "Hands", handsvector, vars.visuals.handsType, &vars.hands_opend, 1);
			this->addcombo(rewidth + 12, y + 50, 90, "Players", chamsvector, vars.visuals.playersType, &vars.players_opend, 1);
		 }
		 
		 if(vars.main.tab == cMainTabs::MiscTab) {
			
			vector<string> clantag;
			clantag.push_back("");
			clantag.push_back("vlone.cc");
			clantag.push_back("Valve");
			
			this->addbutton(rewidth + 12, y + 50, "Enabled", &vars.misc.enabled, 0);
			this->addbutton(rewidth + 12, y + 50, "Bunny hop", &vars.misc.bhop, 1);
			this->textbox(rewidth + 12, y + 50, 170, "Name", m_szCurrentString, 2, 2, &vars.misc_changename);
			this->textbox(rewidth + 12 + 220, y + 50, 170, "Clantag", m_szCurrentString, 3, 2, &vars.misc_clantag);
			this->addbutton(rewidth + 12 + 220, y + 50, "Enable Clantag", &vars.misc.clantag, 3);
			this->addslider(rewidth + 12, y + 50, 90, "Field Of View", vars.misc.fov, 3, 60, 0);
			this->addbutton(rewidth + 12, y + 50, "Auto Strafe", &vars.misc.autostrafe, 4);
			this->addbutton(rewidth + 12 + 170, y + 50, "Circle Strafe", &vars.misc.circlestrafe, 4);
			this->addbutton(rewidth + 12, y + 50, "Airstuck", &vars.misc.airstuck, 5);
			this->addbutton(rewidth + 12, y + 50, "Show Ranks", &vars.misc.showrank, 6);
			
			
			this->addbutton(rewidth + 12, y + 50, "Anti Screenshot", &vars.misc.antiscreenshot, 13, IFlags::IFLAG_WARNING);
			this->addbutton(rewidth + 12 + 170, y + 50, "Anti Untrust", &vars.misc.antiuntrust, 13, IFlags::IFLAG_WARNING);
			
		 }
		 
		 if(vars.main.tab == cMainTabs::HvHTab) {
		 ...
		 antiaim_pitch.push_back("Custom");
			...
			if(vars.misc.aaX == 5)
		 this->addslider(rewidth + 12, y +50, 90, "Pitch", vars.misc.customaa, 1, 180, 0);
			
			if(vars.misc.aaY == 9) {
		 this->addslider(rewidth + 12 + 170, y +50, 90, "Fake Yaw", vars.misc.fakeyaw, 1, 180, 0);
		 this->addslider(rewidth + 12 + 170, y +50, 90, "Real Yaw", vars.misc.realyaw, 2, 180, 0);
			}
			
			this->addcombo(rewidth + 12, y + 50, 90, "Anti Aim X", antiaim_pitch, vars.misc.aaX, &vars.aaX_opend, 0);
			this->addcombo(rewidth + 12 + 170, y + 50, 90, "Anti Aim Y", antiaim_yaw, vars.misc.aaY, &vars.aaY_opend, 0);
			
			this->addbutton(rewidth + 12, y + 50, "Thirdperson (HVH)", &vars.misc.thirdpersonmode, 4);
			this->addbutton(rewidth + 12, y + 50, "At Targets", &vars.misc.attargets, 5);
			this->addbutton(rewidth + 12, y + 50, "Fake", &vars.misc.fakeaa, 6);
		 }
		 
		 if(vars.main.tab == cMainTabs::RemovalsTab) {
			this->addbutton(rewidth + 12, y + 50, "No Recoil", &vars.misc.norecoil, 0);
			this->addbutton(rewidth + 12, y + 50, "No Visual Recoil", &vars.misc.novisualrecoil, 1);
			this->addbutton(rewidth + 12, y + 50, "No Sky", &vars.misc.nosky, 2);
			this->addbutton(rewidth + 12, y + 50, "No Flash", &vars.misc.noflash, 3);
			//this->addbutton(rewidth + 12, y + 50, "World Paint", &vars.misc.worldpaint, 4;
		 }
		 
		 if(vars.main.tab == cMainTabs::PlayerlistTab) {
			if(!bCheckState()) {
		 this->drawstringINDEX(rewidth + 12 + 170, y + 70, Color(0, 0, 0, 255), "Player is not ingame.", 0, false, IFlags::IFLAG_DANGER);
			} else {
		 ...
		 this->addplayerlistbutton(rewidth + 12, y + 50, 90, 20, bPadding, vars.playerTab, info.name, flag);
		 
		 if(bPadding == vars.playerTab) {
		 sprintf(plinfo, "Player info for : %s with index %i", info.name, entity->GetIndex());
		 this->drawstringINDEX(rewidth + 12 + 170, y + 50, Color(0, 0, 0, 255), plinfo, 0, false, flag);
		 this->addbutton(rewidth + 12 + 170, y + 50, "Resolve", &gCorrections[entity->GetIndex()].resolved, 1);
		 this->addbutton(rewidth + 12 + 170, y + 50, "Whitelist", &gCorrections[entity->GetIndex()].whitelist, 2);
		 this->addcombo(rewidth + 12 + 170, y + 50, 90, "Pitch", PitchResolver, gCorrections[entity->GetIndex()].dropdown_x, &vars.playerlist_opend_x[entity->GetIndex()], 3);
		 this->addcombo(rewidth + 12 + 170 + 150, y + 50, 90, "Yaw", YawResolver, gCorrections[entity->GetIndex()].dropdown_y, &vars.playerlist_opend_y[entity->GetIndex()], 3);
		 }
		 bPadding++;
		 }
			}
